...
import type {
	...
	LeaveOptions,
	SuiStackMessagingClientOptions,
	SuiStackMessagingCompatibleClient,
...
import { SuiStackMessagingCall } from './call.js';
import { SuiStackMessagingTransactions } from './transactions.js';
import { SuiStackMessagingBCS } from './bcs.js';
import { SuiStackMessagingDerive } from './derive.js';
import { SuiStackMessagingView } from './view.js';

/**
 * Client extension factory for messaging groups.
 *
 * Requires the base client to already have `suiGroups` and `seal`
 * extensions registered. For a simpler setup that handles all extensions
 * automatically, see {@link createSuiStackMessagingClient}.
 *
 * @example
 * ```ts
 * // Use a single $extend call with all extensions
 * const client = new SuiClient({ url: 'https://...' }).$extend(
 *   suiGroups({ witnessType: `${pkg}::messaging::Messaging`, packageConfig }),
 *   suiStackMessaging({ packageConfig, encryption: { sessionKey }, relayer: { relayerUrl } }),
 * );
 *
 * // Send a message
 ...
 * ```
 */
export function suiStackMessaging<
...
>({
	...
	attachments?: SuiStackMessagingClientOptions<TApproveContext>['attachments'];
	/** Optional recovery transport for fetching messages from an alternative storage backend. */
	recovery?: RecoveryTransport;
}) {
	return {
		name,
		register: (client: SuiStackMessagingCompatibleClient<GroupsName, SealName>) => {
			return new SuiStackMessagingClient<TApproveContext>({
				client,
				groupsName,
				sealName,
				packageConfig,
 ...
 *
 * Requires a SuiClient extended with SuiGroupsClient and SealClient.
 *
 * @example
 * ```ts
 * // Send a message
 ...
 * ```
 */
export class SuiStackMessagingClient<TApproveContext = void> {
	#packageConfig: SuiStackMessagingPackageConfig;
	...
	tx: SuiStackMessagingTransactions;
	view: SuiStackMessagingView;
	bcs: SuiStackMessagingBCS;
	derive: SuiStackMessagingDerive;
	...
	constructor(options: SuiStackMessagingClientOptions<TApproveContext, string, string>) {
		if (!options.client) {
			throw new SuiStackMessagingClientError('client must be provided');
		}
		...
		if (options.packageConfig) {
			...
			switch (network) {
				case 'testnet':
					this.#packageConfig = TESTNET_SUI_STACK_MESSAGING_PACKAGE_CONFIG;
					suinsConfig ??= TESTNET_SUINS_CONFIG;
					break;
				case 'mainnet':
					this.#packageConfig = MAINNET_SUI_STACK_MESSAGING_PACKAGE_CONFIG;
		...
		this.bcs = new SuiStackMessagingBCS({ packageConfig: this.#packageConfig });
		this.derive = new SuiStackMessagingDerive({ packageConfig: this.#packageConfig });
		this.view = new SuiStackMessagingView({
		...
		this.call = new SuiStackMessagingCall({
			packageConfig: this.#packageConfig,
		...
		this.tx = new SuiStackMessagingTransactions({
			call: this.call,
		});
	 ...
	 * @throws {SuiStackMessagingClientError} if the transaction fails
	 */
	async #executeTransaction(transaction: Transaction, signer: Signer, action: string) {
		...
		if (!tx) {
			throw new SuiStackMessagingClientError(`Failed to ${action}: no transaction result`);
		}
	 ...
	 *
	 * @throws {SuiStackMessagingClientError} if no recovery transport is configured.
	 */
